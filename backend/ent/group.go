...
type Group struct {
	...
	// OpenAI compact 成功后是否清除会话账号粘性，以便下一请求重新调度
	OpenaiCompactRebalanceEnabled bool `json:"openai_compact_rebalance_enabled,omitempty"`
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldOpenaiCompactRebalanceEnabled:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field openai_compact_rebalance_enabled", values[i])
			} else if value.Valid {
				_m.OpenaiCompactRebalanceEnabled = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("openai_compact_rebalance_enabled=")
	builder.WriteString(fmt.Sprintf("%v", _m.OpenaiCompactRebalanceEnabled))
	builder.WriteString(", ")
